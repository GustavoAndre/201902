        ...
        void GuardarAdicionar()
        {
            try
            {
                if (MessageBox.Show("¿Esta Seguro que quiere Guardar Nuevo Empleado!!!?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    objProd.IdCategoria = int.Parse(txtCat.Text);
                    objProd.CantidadxUnidad = int.Parse(txtCanxUni.Text);
                    ...
                    objProd.FechaDeRegistro =DateTime.Parse( txtFechaDeIngreso.Text);
                    objProd.HoraRegistro = DateTime.Parse(txthorRegi.Text);
